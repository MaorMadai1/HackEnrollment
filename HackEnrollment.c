...
int* updateArrOfInts(FILE* file) {
    ...
    while (fscanf(file, "%c", &transition) == 1) {  //try fgetc
        ...
        //else if (transition == MOVE_ROW_DOWN) {
        // break;
        //}
        sizeOfArr++;
        paramArr = reallocateIntArr(buildInteger(transition, num), paramArr , sizeOfArr);
    }
    /* ----if the row in file is definitely not empty:
    int intToInsert;
    while (fscanf(hackers, "%d%c", intToInsert , transition)) {
        paramArr = reallocateIntArr(intToInsert, paramArr , ++sizeOfArr);
        if (transition == MOVE_ROW_DOWN) { //break from loop if end of row is reached
            break;
        }
    }
    */
...
IsraeliQueueError enqueueHackers(EnrollmentSystem sys) {
    int k=0;
    Student currentHacker = sys->hackersArr[k];
    while (currentHacker) {
        int* courseNumPtr = currentHacker->desiredCourses;
        Course currentCourse = searchCourse(sys->coursesArr, *(courseNumPtr++));
        while (currentCourse) {
            ...
            currentCourse = searchCourse(sys->coursesArr, *(courseNumPtr++));
        }
        k++;
        currentHacker = sys->hackersArr[k];
    }
    return ISRAELIQUEUE_SUCCESS;
}
...
Student checkPlacementOfHackers(EnrollmentSystem sys) {
    ...
    while (currentHacker) {
        ...
        if (successfulRegistrations < MIN_SUCCESSFUL_REGISTRATIONS) {
...
void printQueuesIntoFile(EnrollmentSystem sys, FILE* out) { //TODO - printed the first and second hackers twice in queue
    ...
    while (currentCourse) {
        ...
        while (nextInLine) {
            //print # digits of IDs
            if (nextInLine->studentID < 100000000) {
                fprintf(out, " ");
                for (int j=1; j*(nextInLine->studentID) < 100000000;j*=ADD_DECIMAL_PLACE) {
                    fprintf(out, "%d", 0);
                }
