...
void IsraeliQueueDestroy(IsraeliQueue q) {
    /*
    while (q->head) {
        void* trash_ptr = IsraeliQueueDequeue(q);
    }
    */
    while (q->head) {
...
static void findPlaceInQueue(Node newNode,IsraeliQueue queue) {
    ...
    while (s != NULL) {
        ...
        if(result == FRIEND && s->friend_counter < queue->friendshipThreshold && friendNode==NULL) {
            friendNode = s;
        }
        else if(result == RIVAL && s->rival_counter < queue->rivalryThreshold && friendNode!=NULL){
            friendNode = NULL;
